...
export var controller_sens = 3
export var rot_speed = 25

var velocity = Vector3.ZERO
var snap_vector = Vector3.ZERO # for slope movement
...
func _unhandled_input(event):
	...
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		rotate_y(deg2rad(-event.relative.x * mouse_sens)) #horizontal camera movement
		spring_arm.rotate_x(deg2rad(-event.relative.y * mouse_sens)) #vertical camera movement
...
func update_snap_vector(): # update snap vector
...
func apply_controller_rotation():
	...
	if InputEventJoypadMotion:
		rotate_y(deg2rad(-axis_vector.x) * controller_sens)
		spring_arm.rotate_x(deg2rad(-axis_vector.y) * controller_sens)
